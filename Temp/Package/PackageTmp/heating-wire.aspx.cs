...
public partial class heating_wire : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("/zh/product/heating-wire");
        }
    ...
    protected DataTable GetCategoryItem(string category, string lang)
    {
        ...
        using (SqlConnection conn = new SqlConnection(webConnectionString))
        {
            ...
            string query = "select p.ID [ID]" +
                " ,p.HotItem [HotItem]" +
                " ,pmc.Name [Name]" +
                " ,pts.ElectricResistance [ElectricResistance]" +
    ...
    protected void PopulateProductListItem(DataTable dt)
    {
        foreach (DataRow dr in dt.Rows)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dr["ElectricResistance"].ToString()))
            {
                HtmlGenericControl divSecondAttribute = new HtmlGenericControl("div");
                if (RouteData.Values["language"].ToString().ToLower() == "zh")
                {
                    divSecondAttribute.InnerText = "電阻: " + dr["ElectricResistance"].ToString();
                }
                else
                {
                    divSecondAttribute.InnerText = "ER: " + dr["ElectricResistance"].ToString();
                }
